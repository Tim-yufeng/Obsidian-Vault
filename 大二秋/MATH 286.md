...
- [x] 286 Quiz 5 ⏫ 📅 2025-12-06 ✅ 2025-12-08
- [x] 286 Quiz 6 ⏫ 📅 2025-12-06 ✅ 2025-12-08
...
这个方程可以解出两个独立的 $r$ 值，分别作为递推公式 (30.8) 的起点，从而得到完整的，两个独立的解

注意找两个关键的参数 $p_{0},q_{0}$ 的时候，我们找的是方程 $x''+p(t)x'+q(t)x=0$ 形式下，$tp(t)$ 和 $t^{2}q(t)$ 的多项式展开的第一项系数，也就是常数项；$p_{1}, p_{2}\dots.$, $q_{1}, q_{2},\dots$ 以此类推

比如说，1/2 阶的 Bessel's equation 的 $p, q$ 系数分别是
$$
p_{0}=1, p_{k}=0 (k\geq1)
$$
$$
q_{0}=-\frac{1}{4}, q_{1}=0, q_{2}=1, q_{k}=0 (k\geq 3)
$$
...
2. 指标方程解出的两个解差值为整数，即 $r_{1}=r_{2}+N, N\in \mathbb{N}$
 第一种情况可以看作是第二种的一个特例；
 但是如果是第二种情况，两个 $r$ 值相差一个正整数，代入较大的一个得到一个解，代入另一个可能得不到一个独立的解；而如果两个 $r$ 相等，那么肯定无法得到两个独立的解

这时候需要使用如下公式来得到另一个解：

![[Pasted image 20251130190849.png]]

注意 $a_{k}(r)$ 这里，我们是将系数 $a_{k}$ 视为关于 $r$ 的一个函数，因为固定了 $k$ 值，它确实可以看作由 $r$ 决定的（可以看课件648-652的例子帮助理解）

于是我们得到了下面这个公式，其中 $a'_{k}(r_{2})=\frac{d}{dr}a_{k}(r)|_{r=r_{2}}$

总结一下这种方法的解题流程：
1.  **第一步：确认奇点类型**
    - **目标**：判断 $t=0$ 是普通点还是正则奇点。
    - **操作**：将方程化为标准形式 $x'' + p(t)x' + q(t)x = 0$。
    - **关键检查**：
        - 计算 $t \cdot p(t)$ 和 $t^2 \cdot q(t)$。
        - 如果这两个函数在 $t=0$ 处是**解析的**（即可以展开为幂级数），那么 $t=0$ 是一个**正则奇点**。**只有在这种情况下，才能使用弗罗贝尼乌斯方法。**
        - 如果 $t=0$ 是普通点（ordinary point），直接用标准幂级数解法。
        - 如果 $t=0$ 是不规则奇点，弗罗贝尼乌斯方法不适用。

2.  **第二步：建立指标方程**
    - **目标**：找到两个指标根 $r_1, r_2$。
    - **操作**：将 $t \cdot p(t)$ 和 $t^2 \cdot q(t)$ 展开为幂级数：
        $t \cdot p(t) = p_0 + p_1 t + p_2 t^2 + \dots$
        $t^2 \cdot q(t) = q_0 + q_1 t + q_2 t^2 + \dots$
    - **关键方程**：指标方程是 $F(r) = r(r-1) + p_0 r + q_0 = 0$。
    - **注意**：$p_0$ 和 $q_0$ 是这两个幂级数的常数项。

3.  **第三步：求解指标方程**
    - **目标**：求出 $r_1$ 和 $r_2$。
    - **关键分析**：
        - **情况一：$r_1 \neq r_2$ 且 $r_1 - r_2 \notin \mathbb{Z}$ (不相差整数)**：直接代入 $r_1$ 和 $r_2$ 分别得到两个线性独立的解 $x_1(t)$ 和 $x_2(t)$。
        - **情况二：$r_1 = r_2$ (重根)**：代入 $r_1$ 得到第一个解 $x_1(t)$。第二个解是 $x_2(t) = x_1(t) \ln t + \sum_{k=1}^{\infty} b_k t^{k+r_1}$。
        - **情况三：$r_1 - r_2 \in \mathbb{Z}$ (相差整数)**：代入较大的根 $r_1$ 得到第一个解 $x_1(t)$。代入较小的根 $r_2$ 可能无法得到第二个独立解。第二个解是 $x_2(t) = \frac{\partial}{\partial r} \left( t^r \sum_{k=0}^{\infty} a_k(r) t^k \right) \Big|_{r=r_2}$。这通常会产生一个 $\ln t$ 项。

4.  **第四步：建立递推关系**
    - **目标**：求出系数 $a_k$ 的递推公式。
    - **操作**：将试探解 $x(t) = t^r \sum_{k=0}^{\infty} a_k t^k$ 代入原方程。
    - **关键**：将方程整理成按 $t^{r+m}$ 的幂次排列。对于每一个 $m \geq 0$，令 $t^{r+m}$ 的系数为零，得到一个关于 $a_m$ 和前面系数 $a_0, a_1, \dots, a_{m-1}$ 的方程。
    - **注意**：$a_0$ 通常是任意的（非零），我们通常设 $a_0 = 1$ 以简化计算。

5.  **第五步：求解递推关系**
    - **目标**：计算出 $a_1, a_2, a_3, \dots$。
    - **操作**：利用递推关系，从 $a_0$ 开始，逐项计算。
    - **注意**：在计算过程中，如果某个 $a_m$ 的表达式分母为零，这可能意味着该级数在 $m$ 处终止（得到一个多项式解），或者与指标根的性质有关。

6.  **第六步：写出通解**
    - **目标**：将所有结果组合成通解。
    - **操作**：根据前面的分析，写出两个线性独立的解 $x_1(t)$ 和 $x_2(t)$。
    - **最终答案**：通解为 $x(t) = c_1 x_1(t) + c_2 x_2(t)$。
### Bessel Function

Bessel's function 的两个解分别对应两条曲线，
一个称为 *Bessel function of the first kind* $J_{\nu}$ 

另一个称为 *Bessel function of the second kind*， 或者叫 *Neumann function* $Y_{\nu}$ 

对于1/2阶的Bessel function：
$$
...
$$
![[Pasted image 20251130221525.png]]


在 $x\to \infty$ 时，Bessel's function 趋于这样一个函数：

![[Pasted image 20251130220957.png]]

## The Space of Square-Integrable Functions

我们考虑像线代中那样，将一个函数 $F(t)$ 表示为一系列函数的线性组合的形式，但是和线代不同的是，这个组合是无穷的，也就是一个级数：

![[Pasted image 20251203193851.png]]

(其中 $\lambda_{n}\in \mathbb{C}$)

而我们要表示的函数 $F$ 可以是任何函数，甚至不一定连续

接下来考虑定义一个无穷维向量空间 $V$, 比如满足在一段区间上连续的所有函数，我们希望找一个基底，使得向量空间内的每个函数都能被唯一表示 （在我们之前学习的线性代数中，找一个无穷维向量空间的基底是不可想象的）

但是因为现在是无穷个函数的线性组合，组合结果等于另一个函数应该写作：
$$
v=\lim_{ N \to \infty } \sum_{n=0}^N \lambda_{n}u_{n}
$$
(无穷多个基底函数 $u_{n}$ 组合表示函数 $v$)

研究极限，就得有一个像样的 norm，即 $||v-\sum_{n=0}^N \lambda_{n}u_{n}||\to0$ as $N\to \infty$

由于在无限维空间中，所有 norm 不再是equivalent 的了（285的内容），我们定义这样一个 norm：
![[Pasted image 20251203195734.png]]
![[Pasted image 20251203195753.png]]

不幸的是, 这个 norm 在 $C([a, b])$ 上是 incomplete 的，但是，我们可以定义一个使得这个 norm 完备的空间，也就是**平方可积函数空间** $L^{2}([a, b])$：
...
- [x] quiz 7 ⏫ 📅 2025-12-13 ✅ 2025-12-09
