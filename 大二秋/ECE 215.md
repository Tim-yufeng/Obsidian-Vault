...
- [x] ECE 215 HW4 📅 2025-11-28 ✅ 2025-11-25
## AC Power Analysis
本章主要是交流电路的功率问题

瞬时功率（Instantaneous power): $p(t)=v(t)i(t)$

假设$v(t)=V_{m}\cos(\omega t+\theta_{v}),i(t)=I_{m}\cos(\omega t+\theta_{i})$
通过积化和差，可以得到一个不随时间振荡的项和随时间振荡的项的和：

![[Pasted image 20251124224434.png]]

![[Pasted image 20251124224620.png]]

**Average Power**:$=\frac{1}{2}V_{m}I_{m}\cos(\theta_{v}-\theta_{i})=\mathrm{Re}\left( \frac{1}{2} V_{m}I_{m}\angle\theta_{v}-\theta_{i}\right)$

$\theta_{v}=\theta_{i}$: pure resistive load
$\theta_{v}-\theta_{i}=\frac{\pi}{2}$: purely reactive load

Average power 相当于电路中只有纯电阻贡献的那部分功率，即：
$P=\frac{1}{2}V_{m}I_{m}\cos(\theta_{v}-\theta_{i})=\frac{1}{2}I^{2}R$

**Maximum Average Power Transfer**:

$Z_{L}=Z_{Th}*$ 时取到最大，此时负载阻值的虚部部分和等效电阻的虚部相抵消，结果是纯电阻

$$
P_{max}=\frac{V_{Th}^{2}}{8R_{Th}} \;\;(R_{Th}=\mathrm{Re}(Z_{Th}))
$$

有效电流/电压，是等效于相同功率的直流电流/电压的值，等于 square root of the mean of the square （平方的 平均值 的 开根号），简写作  rms

![[Pasted image 20251124225707.png]]

对于正弦信号，有：
$$
I_{rms}=\frac{I_{m}}{\sqrt{ 2 }}, V_{rms}=\frac{V_{m}}{\sqrt{ 2 }}
$$
**Apparent power:**  $S=\tilde{V_{rms}}\tilde{I_{rms}}=\frac{1}{2}\tilde{V_{m}}\tilde{I_{m}}=V_{rms}I_{rms}\angle(\theta_{v}-\theta_{i})$

**Power factor:**  $\text{pf}=\cos(\theta_{v}-\theta_{i})$

**Power factor angle:**  $\theta=\theta_{v}-\theta_{i}$
$$
S=P+jQ
$$
...
$$
$$
Z=\frac{\tilde{V}}{\tilde{I}}=\frac{\tilde{V_{rms}}}{\tilde{I_{rms}}}=\frac{V_{rms}}{I_{rms}}\angle\theta
$$
![[Pasted image 20251124231033.png]]

(lagging 指的是 I lags V, leading 就是 I leads V)

在交流电中，我们将功率表示为复数形式，即 Complex power, 这样可以通过复数表示更多的信息

一个复平面的复数可以分解成实部向量和虚部向量的和，对应到 Complex power， 就是将复数功率分解为 Real Power（实部）和 Reactive Power（虚部）

![[Pasted image 20251124231519.png]]

这个三角形称为 Power triangle, 

要注意的一点是，同为功率，只有 $P$ 作为实数部分，用 W 做单位，$S$ 单位是 VA,  $Q$ 单位是 VAR

另一个注意点，就是 $P\neq\frac{V_{rms}^{2}}{R}, Q\neq\frac{V_{rms}^{2}}{X}$， 各自计算它们时，都必须用电流计算

 **Conservation of AC Power**:  Real Power 和 Reactive Power 分别守恒
**Power factor correction**: 通常是在负载两端并联一个电容，目的是让 pf angle 尽可能趋于零
通过并联一个电容，干路多出一份容性电流，即 I leads V 的电流平衡了感性的负载（大多数实际场合的负载都是 Reactive 的），
![[Pasted image 20251125093959.png]]

其实加上这一份容性电流，电路的总电流是减少了，因而此举可以减少传输损耗

从 Power triangle 的角度，在相同 P 的条件下，减小 Q 也是我们需要的

![[Pasted image 20251125094255.png]]

## Three-Phase Circuits

区分两种 phase order，分别是 abc 和 acb，它们的相位关系有差别：

![[Pasted image 20251127152906.png]]

三相交流电的电源连接和负载连接有四种方式：
![[Pasted image 20251127153023.png]]

### Balanced Wye-Wye Connection
如果3个负载平衡，那么由于相位各相差120, 三段电流会在中间那根线相互抵消为零，于是这根线的电阻可以忽略，电路简化：
<p align="center">
  <img src="Pasted image 20251127153322.png" width="300"/>
  <img src="Pasted image 20251127153340.png" width="340"/>
</p>

注意区分 Phases/Lines voltage/current 的区别

在 Balanced Wye-Wye Connection 中，我们有：
1. Line voltage $V_{L}=\sqrt{ 3 }V_{p}$ Phase voltage（仅比较大小）
2. 相位上，Line voltage 领先 Phase voltage $30^{\circ}$
3. Line current = Phase current

![[Pasted image 20251127155025.png]]

### Balanced Wye-Delta Connection

<p align="center">
  <img src="Pasted image 20251127160040.png" width="320"/>
  <img src="Pasted image 20251127160057.png" width="320"/>
</p>

1. 电压关系和 Wye-Wye 一样，$V_{L}= \sqrt{3}V_{P}\angle30$
2. 电流：线电流是相电流的$\sqrt{3}$倍，相位滞后其 $30^\circ$

![[Pasted image 20251127160259.png]]

### Balanced Delta Delta Connection

![[Pasted image 20251127160524.png]]

### Balanced Delta Wye Connection

![[Pasted image 20251127160557.png]]

- [x] 215 HW 5 ⏫ 📅 2025-12-10 ✅ 2025-11-29

## Magnetically Coupled Circuits

本章的主角是电感线圈，关于电感，我们之前就知道如何计算其自感电压：
$$
v=L\frac{di}{dt} 
$$
根据这个公式，我们进而得到，在一个频率 $\frac{\omega}{2\pi}$ 的交流电路中，一个自感线圈相当于阻值 $j\omega L$ 的电阻

现在研究两个电感线圈的互感，具体推导见课件(Ch13 6-8), 这里直接放结论：
$$
v=M\frac{di}{dt}
$$
其中 $M$ 称为 mutual inductance，对两个线圈都是一样的，永远是正值

互感现象的条件：两个线圈靠的足够近，而且电流随时间不断变化

### Dot convention

用来清晰的表示线圈缠绕方向的一种表示方式；简单来说，就是给两个线圈标注的两个点，需要做到：==电流从这两个点流入，两个线圈产生的磁场方向相同==：
 
<p align="center">
  <img src="Pasted image 20251202164806.png" width="320"/>
  <img src="Pasted image 20251202164817.png" width="320"/>
</p>

由 Dot 的位置如何判断互感电压的方向：

一种方法是，==考虑通过线圈1感应线圈2：如果电流1从线圈1的标点位置流入，那么互感电压在线圈2的标点位置就是 +; 
反之，若在标点位置流出，互感电压在线圈2的标点位置就是 -；==

![[Pasted image 20251202165227.png]]

<p align="center">
  <img src="Pasted image 20251202165300.png" width="320"/>
  <img src="Pasted image 20251202165324.png" width="320"/>
</p>

还有一种方法，就是考虑互感电压和自感电压相同还是相反，如果两个线圈的电流同时从标点流入或流出，则相同；否则相反：
![[Pasted image 20251202165529.png]]

互感现象也可以发生在，两个线圈位于同一个电路中：

<p align="center">
  <img src="Pasted image 20251202165552.png" width="320"/>
  <img src="Pasted image 20251202165628.png" width="320"/>
</p>
### Energy in Coupled Circuit

![[Pasted image 20251204164427.png]]

对于这个电路的能量（功和功率），我们拆成两步求解，其中 Step1 的功率和功分别是:
$$
p_{1}=v_{1}i_{1}=L_{1}\frac{di_{1}}{dt}i_{1}
$$
$$
w_{1}=\int p_{1}dt=\frac{1}{2} L_{1}I_{1}^{2}
$$
($i_{1}$ 是瞬时电流值，$I_{1}$ 是最终电流)

Step2：
$$
p_{2}=\left( M_{12}\frac{di_{2}}{dt} \right)I_{1}+\left( L_{2}\frac{di_{2}}{dt}  \right)i_{2}
$$
$$
w_{2}=M_{12}I_{1}I_{2}+\frac{1}{2}L_{2}I_{2}^{2}
$$
于是我们得到公式，总功 $w$ 应该是
$$
w=\frac{1}{2} L_{1}I_{1}^{2}+\frac{1}{2}L_{2}I_{2}^{2}+M_{12}I_{1}I_{2}
$$
（$M_{12}=M_{21}=M$)

$M$ 可以为负，正负取决于两边的电流是否同时流入或流出标点处

### Coupling coefficient k

<p align="center">
  <img src="Pasted image 20251204165436.png" width="320"/>
  <img src="Pasted image 20251204165446.png" width="320"/>
</p>


### Linear Transformer

重点是一种将分开的两个线圈等效为同一个电路中的线圈（电感）

![[Pasted image 20251204170545.png]]

比如图中所示的线圈可以等效为 T 形或 $\pi$ 形：

<p align="center">
  <img src="d6aa74b86f5f7d815521a567a259085b.jpg" width="320"/>
  <img src="c5b41f52c3452a89e1c0dc8dc222845b.jpg" width="320"/>
</p>

注意我们构建这个等效模型的时候，电流 $I_{1}, I_{2}$ 的方向（一起流入标点）是关键的，在等效的时候也要先将实际电路的电流化为一起流入标点，否则计算结果会出问题

### Ideal Transformer

**要求:**  coupling coefficient k=1, 没有电阻($R_{1}=R_{2}=0$)

![[Pasted image 20251204171238.png]]

理想变压器中，两个线圈接受的磁通量相等，因此我们可以列出熟悉的电压、电流和线圈匝数的关系式（高中内容）
$$
v_{1}=N_{1}\frac{d\phi}{dt}, v_{2}=N_{2}\frac{d\phi}{dt} 
$$
$$
\frac{v_{2}}{v_{1}}=\frac{N_{2}}{N_{1}}=n, \frac{i_{2}}{i_{1}}=\frac{N_{1}}{N_{2}}=\frac{1}{n}
$$
但是有的时候 $n$ 要取负，判断方式还是根据标点：
	1. 如果电压 $V_{1},V_{2}$ 在==标点处同正或同负==，那么用 +n , 否则是 -n
	2. 如果电流 $I_{1}, I_{2}$ ==一起流入或流出标点==，那么用 ==-n ==, 否则是 ==+n==

![[Pasted image 20251205203614.png]]

两个线圈处的 Complex Power 是相等的：
$$
S_{1}=V_{1}I_{1}^*=V_{2}I_{2}^*=S_{2}
$$
这也对应了，理想变压器在电压转换过程中，没有能量损耗的性质

![[Pasted image 20251205203857.png]]

**Input Impedence**: 副线圈的电阻值等效到原线圈的阻值大小，乘以 $\left( \frac{n_{1}}{n_{2}} \right)^{2}$：

![[Pasted image 20251205204144.png]]

两种等效电路，将分隔开的理想变压器化成一体的电路，一种是副线圈并入原线圈，另一种是原线圈并入副线圈：

<p align="center">
  <img src="Pasted image 20251205212628.png" width="320"/>
  <img src="Pasted image 20251205212645.png" width="320"/>
</p>

This reflection approach only applies if there are no external connections between the primary and secondary windings. When we have external connections between the primary and secondary windings, we simply use regular mesh and nodal analysis