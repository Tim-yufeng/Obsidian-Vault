...
当我们有许多寄存器并列在一起，但是每次只需要选择一个进行读取或写入时，Register File是一个比无脑并联寄存器更高效、合理的设计
下图展示了一个寄存器堆的设计：
...
回到 FSM 的优化精简，我们列出 state table，然后观察，如果有两个或多个状态，它们的不同输入输出下的==输出和未来状态==都一模一样，那它们就可以合并为一个状态；我们将全部合并后的状态们列成一张新的、精简的 state table，然后据此画出 state diagram 和设计相应电路
...
上面这个例子我们是用 Mealy 电路来设计的，我们也可以设计出 Moore 的形式

对比同一个电路用 Mealy 和 Moore 实现的区别：

![[Pasted image 20251117161941.png]]

总结来说，Mealy 通常更小、更快，但是不够稳定
在实际工程应用中，Moore 其实用的更多

Mealy 和 Moore 都有自己的一个标准框架结构：

![[Pasted image 20251117184651.png]]

现在我们考虑 Mealy 的Verilog实现，其实和 Moore 大体上差不多，只是最后一步，也就是赋值输出，我们考虑的不只是状态，而是状态和输入都要考虑

有趣的是，Mealy 和 Moore并不总是泾渭分明，有时候也可以被揉进同一个电路里面，像下面这个例子：

![[Pasted image 20251117185034.png]]

接下来我们介绍另一种减少状态数的优化方法，这种方法是前面那张方法失效（放眼整个state table 找不出两组输出和next state都相同的状态）
的时候，给出可能进一步优化的方法

以下面这个无法直接消去状态的电路为例：
![[Pasted image 20251117185551.png]]

我们降低标准，不求输出和next state 都一模一样，只要求输出一样；在下面这张“状态两两对比表”中，将输出不同的格子打叉，相同的留着:
![[Pasted image 20251117185838.png]]

接着我们研究next state，没有打叉的格子，两个括号分别代表了输入1和0的情况下，被对比的两个状态各自的 next state 分别是什么（它们分别去往何方）

最后，如果一个格子内的next state “坐标” 位于没有被打叉的格子，那么这个格子保留，被对比的两个状态可以合并；否则打叉，无法合并
在这个例子中两个格子都可以保留

最后画出合并后的 diagram：

![[Pasted image 20251117190316.png]]

<span class="green"> 这种方法的核心思想就是：输出不同的两个状态一定无法合并，但是输出相等、去处不同的两个状态还有研究的余地，因为合并后可能去处就相同了；可以理解为，你无法根据输出的不同笃定的判断它们的去处一定不同，那么它们就可以相同；也可以理解为，我们假设它们的去处相同，结果发现它们真的相同了！</span>

这个例子是对 Moore 的优化，如果优化对象是 Mealy 呢？
情况略有变化，但是变化也不大，主要就是我们要比对的输出变成了两个（或者多个，取决于输入个数）
![[Pasted image 20251117190929.png]]
差点忘了说，我们用来打叉的这个表格（？）称为 implication table

除了这两种基于减少状态数的优化方法，还有基于**设计状态编码方式**的优化方法（不难理解，因为编码方式不同，最后的逻辑电路肯定不一样）

一种方式是，原本我们四个状态，按照计数的顺序编码：00，01，10，11
现在我们用 K-map 的方式：00，01，11，10 总之就是保证相邻只变化一个 bit
这张编码方式甚至还有一个名字，叫 Gray Code

还有一种神奇的编码方式，就是我们不再追求比特的利用率，直接一个状态用一个比特，让编码比特数等于状态数，类似于 Decoder ：

<p align="center">
  <img src="Pasted image 20251117191802.png" width="320"/>
  <img src="Pasted image 20251117191822.png" width="320"/>
</p>

这种编码方式被称为 One-Hot Encoding

缺点当然是，浪费了比特就意味着更大的寄存器，更多的 DFF；但是优点是，逻辑电路部分极其简洁，甚至几乎没有用到逻辑门，这使得电路运行起来会更快
<span class="green"> 个人的理解，这是因为，Input 的状态是四个状态（好巧这个FSM竟然没有输入？），意味着每种情况都只有一个1，那么对应到输出端（next state和FSM输出），基本只靠接线就能实现，确实不需要复杂的逻辑组合
 </span>

前面的 FSM 设计中，多余比特表示的状态我们是当作 don't care 处理的，因为这些状态理论上不可能发生，但是工程上我们还是会认真考虑这些 Unused states 究竟取什么值。从而完成一个 Complete FSM

- [x] 270 HW 8 ⏫ 📅 2025-11-26 ✅ 2025-11-25

# RTL Design- Design of a Digital System

本章要探讨的是，如何将之前的 FSM 上升到一个更复杂的 Digital System，这个系统由两部分组成，Controller 和 Datapath

其中 Controller 是一个 FSM，用于描述目标系统的功能，操纵全局的运作；Datapath 完成具体的信号、数据处理、计算、存储等任务，受到 Controller 的操控

![[Pasted image 20251125164807.png]]

RTL Design 的流程是：
1. 首先用 diagram 描述出目标系统的运作流程，但是和 FSM 不同的是，这个流程涉及的输入和输出，触发条件不再是简单的几个比特，逻辑组合，而是许多比特组成的更复杂的数据、信息，和需要借助 Building Block 来实现的复杂逻辑及运算；因此这个东西不再叫 FSM，而是称为 high-level state machine, 简称 HLSM
2. 然后设计 datapath, 就是实施第一步中，那些复杂的数据处理方法，通过 Building Block 的使用
3. 连接 Controler 和 Datapath
4. 将 HLFM 变成 FSM，因为 step2 中的运算实施和 step3 的连接逻辑已经将复杂运算变成简单 bit 了，可以用 FSM 方法来设计 Controller 的逻辑了
...
我们知道，再一个全加器(Full Adder)中，carry的计算逻辑是 $ab+(a\oplus b)c$, 现在我们要换种方式表示它们，定义 P (Propagate), G (Generate) :
P=$a\oplus b$
G=$ab$
于是carry的值就可以表示为 P, Q 和前一个carry c 的逻辑组合：

![[Pasted image 20251201230129.png]]

接下来以一个4位的加法器为例，其中 $c_{0},c_{1}, c_{2}, c_{3}$ 分别代表4个carry，$c_{0}$ 是最开头的那个 cin.  然后 cout, $s_{0},s_{1}, s_{2},s_{3}$, 是最后的进位和计算结果的4位
下图展示了这个过程，注意看，我们把 $c_{1}, c_{2}, c_{3}$ 都用 $c_{0}$ 表示了，以节省 delay

![[Pasted image 20251201230512.png]]
下面复杂的逻辑门就是由 $c_{0}$, P0, P1, P2, P3, G0, G1, G2, G3 求出 cout, $c_{1}, c_{2},c_{3}$ 的逻辑

![[Pasted image 20251201230551.png]]

上方圈出的方块内部被称为一个 SPG block, 一个4位的加法器上方需要4个这样的 SPG block
可以看到，所有 P, G 的产生只要一个 gate delay，然后 P, G 和 $c_{0}$ 又需要两个 delay 来算出 $c_{1}, c_{2}, c_{3}$ 和 cout，最后，再要一个 delay 来从 $c_{1}, c_{2}, c_{3}$ 算出 $s_{0},s_{1}, s_{2},s_{3}$
总共是4个delay

为了将相同的方法应用到多个 Adder 合并的更多比特加法器上，我们给原来的 CLA 增加两个输出：P，G：

![[Pasted image 20251202092840.png]]

![[Pasted image 20251201232202.png]]

1个delay得到上面那层的所有P，G，然后2个delay得到 P0, G0, 然后由 $c_{0}$, P0, G0 得到 下面那层的$c_{1}, c_{2}, c_{3}$需要2个delay，有了$c_{1}, c_{2}, c_{3}$，其它上层的 Adder 还需要3个delay（为什么不是4个？因为上层的P，G早就已经求出来啦），计算出所有的结果 $s$ 

### 乘法器Multiplier

首先，二进制的多位乘法分为 multiplicand（上面那个）和 multiplier（下面那个）， multiplier每一位分别和 multiplicand做乘法，也就是AND运算：
![[Pasted image 20251202093701.png]]

实现这个运算，一种优化后的方式是使用时序电路，定义一个叫 running sum 的量，不断将AND的结果（partial product）加给这个量，不断更新这个running sum 直到得到最终答案

![[Pasted image 20251202094527.png]]

我们用 RTL 的方法设计这个 sequential circuit:

![[Pasted image 20251202094600.png]]
![[Pasted image 20251202094653.png]]

# Timing Issues

### Gated Clock

首先探讨的是，逻辑门在Synchronous的电路中造成的延迟问题。比如一个 Clock Divider，其Divider的功能由一个 Combinational circuit 实现，但是里面的逻辑门会造成一定的 gate delay, 从而造成我们称为 Clock skew 的现象，导致本应 Synchronous 的电路表现得 Asynchronous

![[Pasted image 20251203204632.png]]

解决办法的添加一个 DFF 作为 Synchronizer, 这个 Synchronizer 的作用不是彻底消除延迟（因为 DFF 自己就包含逻辑门，会产生一定的延迟），而是为了把本来取决于逻辑电路设计的延迟长度控制住，固定为一个 DFF 的延迟
 
<p align="center">
  <img src="Pasted image 20251203204950.png" width="320"/>
  <img src="Pasted image 20251203205004.png" width="320"/>
</p>

### Timing Requirements for FSM 

一个 FSM 的工作流程是这样的：一个时钟上升沿出现，Combinational logic 部分执行逻辑运算，根据 Present State 和 Input 计算出 Next State 和 Output，然后静静等待下一个时钟上升沿的出现

![[Pasted image 20251203205358.png]]

但是这个流程的正常运行有个条件，就是 Combinational logic 部分执行逻辑运算的时间（delay）必须小于两个时钟信号之间的 time interval（否则就出大问题了）

于是我们知道，时钟信号频率的选取是有讲究的，一方面，我们希望频率更大，这样同样的时间 FSM 可以执行更多的工作循环，这台机器运转的速度就更快；另一方面，我们又必须保证频率不会大到导致时钟信号间隔还不够逻辑部分完成计算

这也告诉我们，如果能做到减小 Combinational logic 部分的 time delay，就能有效提升 FSM 的速度

### Setup time and Hold time

对于一个 Flip Flop 来说，数据的传入不是一瞬间就能完成的，而是需要在时钟信号上升沿的前后都有一小段时间间隔来稳定信号，在上升沿之前的这段时间间隔称为 Setup time, 在上升沿之后的称为 Hold time，简单来说：一个信号要想成功的输入 FF，在时钟信号来之前，你得有时间准备，在时钟信号出现后，你得坚持一小会儿不能马上走开

![[Pasted image 20251203210252.png]]

这也是为什么，在 Testbench 设计的时候，很避讳出现 在时钟信号上升沿发生输入信号变化的操作，像下图所示：

![[Pasted image 20251203210408.png]]

如果真的发生这种情况了会怎么样？毕竟在现实中，很多数字产品是 Synchronous的，但是使用它们的人是Asynchronous的（？）这种情况被称为 **Setup/hold time violation**
导致的后果就是，这个 FF 会进入一个不稳定的状态，称为 **metastable state** 输出既非零又非一，虽然最终会稳定下来， 但是稳定到什么值，多久稳定，都是不确定的
![[Pasted image 20251203210903.png]]

为了避免这种violation，常规的做法包括，减小时钟信号频率，减小 combinational logic 的延迟，以及将 combinational logic 拆解成更小的步骤，这样每个步骤一个循环完成，所需的延迟就小多了

### Asynchronous input and synchronizer

外界输入的突如其来可能造成 Setup/hold time violation 的问题，此外，内部信号路径的长短不一甚至都会造成问题，即 **Different fanout path delays** ,为了避免这种问题，我们甚至会设计这样的树状结构：
![[Pasted image 20251203212239.png]]

为了避免外界突如其来的输入可能让 synchronous system 陷入 metastable 的处境，我们在系统的前面加上一个保护作用的 DFF，称为 synchronizer, 它的作用就是：因为和后面的系统共用一个时钟信号，每次外界有输入，必须先经过这个 DFF，而 DFF 内部的延迟远大于 hold time，等信号通过 DFF 的时候，已经可以安全进入系统，不会引起 hold time violation 

<p align="center">
  <img src="Pasted image 20251207145403.png" width="320"/>
  <img src="Pasted image 20251207145424.png" width="320"/>
</p>

现在系统是被保护了，但是前面的这个 DFF 也是一个 synchronous 的元件，有可能会陷入 metastable，这种情况被称为 **synchronizer failure**
要从synchronizer failure恢复出来，一种方法是默默等待，但是回复时间是不可知的；还有一种办法就是直接 reset

避免这个的方法，除了 improve DFF 和 clock period，就是在多一个 DFF，双重 synchronizer 保障，来降低 synchronizer failure 发生的概率：

![[Pasted image 20251207145916.png]]

第二个 DFF 的加入的影响：如果第一个 DFF 不幸陷入非稳态，这个非稳态在一个时钟周期内就结束了，那么不会对后面造成任何影响；
但是如果超过一个周期，那么第二个 DFF 也会跟着陷入非稳态（第一个DFF的非稳态信号就会被第二个 DFF 检测到），于是再次 synchronizer failure

所以说，增加更多的 DFF 是无法彻底消除可能的 synchronizer failure 的（synchronizer failure的概率无法被降低至零），但是可以增加非稳态持续时间的容忍度，从而降低整个系统陷入synchronizer failure 的概率

### Push Button

对于开关，按钮等，其状态在一瞬间改变后，往往会经历一段非稳态的“反复弹跳”过程，这段过程的延续时间是未知的，通常在几毫秒

![[7722ecb27ff3ca5f2450fbc869f81f19.jpg]]

一种解决办法是应用 SR latch：
![[0c2c45b43acceef3fdb7be0148562f44.jpg]]
根据图示电路，开关打向哪一边，哪一边**从1变成0**

原理如这个 truth table 所示：因为switch bouncing的存在，开关打向一边，比如从0到1，会先碰到1，然后弹回来（介于1，0之间，不会回到0），来回横跳，不断碰到1在弹回来，直到最终稳定；
那么经这样设计后，开关先碰到1，之前的状态是 Q=0, 碰到1是 S=0, R=1, 于是 $Q^+$ 被设成 1，弹回来后，S=R=1，Q=1，于是 $Q^+$ 被 hold 在了1，再次碰到1，S=0, R=1，当前状态 Q=1, $Q^+$ 仍然是 1

另一种办法是使用 DFF，通过时钟信号来消除“反复弹跳”带来的 noisy 信号，但是这对时钟频率的设计提出了要求，如果时钟过快，可能会捕捉到这些我们希望除去的noisy信号

![[24ca5cb09dcb173865faaa171dfd18ef.jpg]]

# Memory and PLD

Random Access Memory, 简称 RAM，其运行原理和寄存器堆非常相似，都是通过地址可以读取、写入数据，地址的比特数决定了有多少行数据；

![[0b62925f556e169b87d1d5c613a98b01.jpg]]
和寄存器堆的区别在于，首先RAM更大，存储效率也更高，

如图所示，我们有一个双向的32比特 bus，称为 I/O，一个address写入口，和寄存器堆一样，然后 rw 决定是读取还是写入，en 就是enable

下面是一个 RAM 的内部结构，最左边是一个 decoder，将地址信息转化成控制不同行的 enable 的信息，类似寄存器堆控制各个 tri-state-buffer，后面的每个方块都存储一个 bit，称为 RAM cell

![[eed081ab7363e31b19cc3f01f2404ef3.jpg]]

进入到这个 RAM cell 的内部，可以看到一种和寄存器截然不同的数据存储模式，它是通过一对首尾相接的 inverter，构成一个 loop，将数据“困”在里面，实现存储效果；
该loop左侧永远是信息本身，右侧永远是信息的相反，通过 word enable 控制两侧的 transistor 来控制信息是否被外界读取

![[3fd968977681255f0389d063a59ce365.jpg]]


读取的时候，通过利用外界信息和内部为零的一侧产生的低电压现象（出现在哪一侧）判断出该cell的原本存储信息，至于对存储信息的可能影响，有办法解决的（但是没讲）

![[8df4ae03247a490bc10b1e342bc8f939.jpg]]
