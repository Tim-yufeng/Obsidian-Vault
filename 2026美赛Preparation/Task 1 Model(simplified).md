...
物资到达顶点锚点（Apex Anchor）后，需由“地月转运船”（Space Tug）接力运送至近月轨道（LLO）。

定义决策变量 $n^{EM}_t \in \mathbb{Z}_{\ge 0}$ 为第 $t$ 年执行的转运任务次数。在此简化模型中，我们假设深空转运船的供应是充足的，不受船队规模限制，仅需满足载荷匹配：

- **载荷匹配**：设单次转运标准载荷为 $q^{EM}$，则必须安排足够的转运班次以承载该年物资：
...
2. **转运成本（段B）**：
    ...
    该段成本与转运的总质量（含推进剂与结构）成正比。我们将复杂的火箭方程线性化为“固定+变动”模式。为简化模型，我们假设该技术已相对成熟，单位成本在规划期内保持稳定：
    
    - $C^{EM}_{\text{fix}}$：单次转运的固定任务成本。
        
    - $\alpha_{EM}$：单位载荷的动态成本系数（常数）。
        
    
    $$C_{\text{trans}}(t) = C^{EM}_{\text{fix}} \cdot n^{EM}_t + \alpha_{EM} \cdot m^{SE}_t$$

综上，SES 的总成本为：

$$C_{SE} = \sum_{t \in \mathcal{T}} \left[ \left( c_{\text{climb}} \cdot m^{SE}_t + f_{SE} \cdot z_t \right) + \left( C^{EM}_{\text{fix}} \cdot n^{EM}_t + \alpha_{EM} \cdot m^{SE}_t \right) \right]$$
...
2. **基地有效容量限制**：
    ...
    综合考虑基础设施规模，定义 $L_i$ 为基地 $i$ 的**年度有效最大发射次数**（Effective Annual Launch Cap）。若基地启用，则发射次数不能超过此上限：
...
$$\begin{aligned} \min \quad & J = w_C \frac{C_{SE} + C_R}{C_{\text{ref}}} + w_T \frac{\sum z_t}{T_{\text{ref}}} \\ \text{s.t.} \quad & \\ \text{(1) 总需求:} & \quad \sum_{t \in \mathcal{T}} \left( m^{SE}_t + \sum_{i \in \mathcal{I}} m_{i,t} \right) \ge M_{\text{tot}} \\ \text{(2) 完工逻辑:} & \quad m^{SE}_t + \sum_{i \in \mathcal{I}} m_{i,t} \le \overline{M} \cdot z_t, \quad z_t \ge z_{t+1} \\ \text{(3) 电梯约束:} & \quad m^{SE}_t \le C_{SE}(t) \\ & \quad m^{SE}_t \le q^{EM} n^{EM}_t \\ \text{(4) 火箭约束:} & \quad m_{i,t} \le \eta_i q \cdot n_{i,t} \\ & \quad n_{i,t} \le L_i \cdot y_i \end{aligned}$$
